...
/*VERSIÓN 2: El semáforo parpadea durante el tiempo de encendido de la luz roja
...
//TimePoints
  ...
  unsigned long T4 = T3 + 15;

  int blinker = time_clock % 2;
  ...
  else if ((T3 < time_clock) && (time_clock <= T4))
  {
    digitalWrite(greenPin, LOW);
    digitalWrite(yellowPin, LOW);
    digitalWrite(redPin, HIGH);
